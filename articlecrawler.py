...
class ArticleCrawler(object):
    ...
    def crawling(self, category_name):
        ...
        target_urls = self.make_news_page_url(url_format, self.date['start_year'], self.date['end_year'],
        ...
        for index, url in enumerate(target_urls):
            if index % 100 == 0:
                print(index, ' : ', url, round(index/len(target_urls)*100, 2), '%')
            ...
            for content_url in post_urls:  # 기사 url
                ...
                try:
                    ...
                    time_list = re.findall('<span class="t11">(.*)</span>', request_content.text)
...
#2020123추가       #Reaction추가
                    ...
                    reaction_url = 'https://news.like.naver.com/v1/search/contents?q=NEWS[ne_{}_{}]'.format(oid, aid)        #웹에서 동적으로 받아와짐 => 기사의 oid, aid 활용하여 동적접근
                    reaction_req = requests.get(reaction_url)
                    reaction_html = reaction_req.text

                    reaction_json = json.loads(reaction_html)['contents'][0]['reactions']

                    reactionCountList = [0, 0, 0, 0, 0]
                    reactList = np.array(['like', 'warm', 'sad', 'angry', 'want'])

                    for countIndex, reaction in enumerate(reactList):

                        if reaction not in [i['reactionType'] for i in reaction_json]:  # 값이 없는 리액션은 넘어감
                            continue

                        reaction_index = [i['reactionType'] for i in reaction_json].index(reaction)
                        reactionCountList[countIndex] = reaction_json[reaction_index]['count']

                    timestamp = int(str(json.loads(reaction_html)['timestamp'])[:10])
                    # CSV 작성
                    writer.write_row([time, category_name, text_company, text_headline, text_sentence, content_url, modify_time, reactionCountList, timestamp])
...
if __name__ == "__main__":
    ...
    Crawler.set_category('사회', '정치', '경제') #set Category
    Crawler.set_date_range(2019, 1, 2019, 12)   #set start y/m~ end y/m 
    Crawler.start()

    #2020 Article
    Crawler = ArticleCrawler()
    Crawler.set_category('사회', '정치', '경제')
    Crawler.set_date_range(2020, 1, 2020, 12)
    Crawler.start()

# Using multi-processing
# this code 6 process running
